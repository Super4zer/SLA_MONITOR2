...
// 3. Pengecekan manual untuk memastikan session kedaluwarsa secara presisi
if (isset($_SESSION['LAST_ACTIVITY']) && (time() - $_SESSION['LAST_ACTIVITY'] > $timeout_duration)) {
    session_unset();     // Bersihkan variabel session
    session_destroy();   // Hancurkan session
    session_start();     // Mulai session baru yang kosong
}
// Perbarui waktu aktivitas terakhir pengguna dengan waktu saat ini
...
/* Responsive Login Page Adjustments */
@media (max-width: 768px) {
    body, html { margin: 0; padding: 0; overflow-x: hidden; }

    form[name="frmlogin"] { width: 100%; display: flex; flex-direction: column; align-items: center; }
    form[name="frmlogin"] > br { display: none; }

    /* Bordered login box: biarkan lebar alamiah (max 340px), centered */
    ...
    /* Inner td & table: ikuti lebar bordered box */
    td[width="330"] {
        width: 100% !important;
        box-sizing: border-box;
    }
    table[background="index_files/bgentrikecil.jpg"] {
        width: 100% !important;
        background-size: cover;
    }

    /* Form fields full width */
    table[width="284"] { width: 100% !important; }
    .form-control { width: 100% !important; box-sizing: border-box; }
    input[type="submit"].btn { width: 100% !important; margin-top: 10px; }
    ...
    <td height="400" valign="top" class="clsText"> 
      <table class="clsText" width="100%">
        ...
        if ( islogin() != true){
         ...
         <div style="background-color: #f2dede; color: #a94442; border: 1px solid #ebccd1; padding: 10px 15px; margin-bottom: 10px; border-radius: 4px; text-align: center; max-width: 330px;">
           <i class="fa fa-exclamation-circle"></i> <strong>Login Gagal!</strong> ID User atau Password salah.
         </div>
         <?php } ?>
         <br>
        ...
        <?php }?>
              <form name="kehadiran" method="POST" action="kehadiran.php">
                ...
                if (islogin()==true){
                    ...
                    echo "<div id='home-logo' style='text-align:center;'><img src='Logo.jpg' style='max-width:100%;height:auto;'/></div>";
                }
